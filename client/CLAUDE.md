...
## Microinterações reutilizáveis (US-042)
Duas classes globais em `client/src/index.css` cobrem motion comum em qualquer feature nova — use direto, não recrie animações por componente:
- `.route-fade` — fade 200ms (`opacity 0→1`). Aplicada uma única vez em `App.tsx#RoutedApp` num wrapper keyado por `location.pathname`. Toda rota nova herda o fade automaticamente; **não** aplique manualmente em páginas. O remount via `key` torna a transição interruptível (back/forward rápido cancela e reinicia).
- `.list-enter` — entrada escalonada (`opacity 0→1` + `translateY(8px→0)`, 280ms, ease-out). Use em items de lista que se beneficiam de entrada cascata (ex: cards de histórico, recomendações). Aplique a classe **e** uma inline `style={{ ['--enter-index' as string]: idx }}` para escalonar 30ms por item via `animation-delay: calc(var(--enter-index, 0) * 30ms)`. CSS animations só disparam ao montar/recriar o elemento — re-renders não retriggeram, satisfaz "somente na primeira renderização" sem hooks de detecção.

Hover/active microinteractions já vivem em `.ui-button` (qualquer variante exceto `ghost`): hover dá `translateY(-1px) + shadow-md`, active retorna a `translateY(0) + shadow-xs`. Use a `Button` da biblioteca em vez de estilizar `<button>` na unha — você ganha o efeito "press" gratuitamente e mantém consistência. Para novos controles que precisem do mesmo affordance, herde da classe `.ui-button` ou replique apenas a transição `transform var(--duration-fast) var(--ease-standard) + box-shadow ...`.

Spinner do `Button loading` é SVG inline com `@keyframes ui-spin` rotação pura — `prefers-reduced-motion` global neutraliza a rotação automaticamente; o `aria-busy="true"` no `<button>` carrega a semântica de "ocupado" para leitores de tela.

Regras duras (auditadas em US-042; manter):
- **Nunca** `transition: all` — sempre liste propriedades.
- **Nunca** anime `width/height/top/left/right/bottom` — use `transform: scaleX/scaleY/translate*` ou `clip-path`. Para barras de progresso, `transform: scaleX` com `transform-origin: left center` (ver US-037/US-038 patterns).
- Animações novas usam apenas `opacity` e `transform` — o reset global `prefers-reduced-motion` em `*, *::before, *::after` (em `index.css`) zera `transition-duration`/`animation-duration`/`animation-iteration-count` automaticamente. Não repita o `@media` em CSS novo.
- `transform-origin` correto em qualquer animação de "abrir" (origem = onde o usuário clicou). Translate puro não precisa de origin (é uniforme).
